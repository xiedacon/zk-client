...
|                                v             '                           '
|                         +--------------+     ' +-----------------------+ '
|           +-----------> |  CONNECTING  | --> ' | AUTHENTICATION_FAILED | '
|           |             +--------------+     ' +-----------------------+ '
|           |                    |             '                           '
|           |                    |             +- - - - - - - - - - - - - -+
|           |                    v                           |
...
ConnectionManager.prototype.onSocketClosed = function (hasError) {
    ...
    case STATES.AUTHENTICATION_FAILED:
        ...
        break;
    default:
        // While retry connet, move pending packets back to packetQueue
        this.packetQueue.queue = this.pendingQueue.concat(this.packetQueue.queue);
        this.pendingQueue = [];
        errorCode = Exception.CONNECTION_LOSS;
        retry = true;
    }
...
ConnectionManager.prototype.onSocketConnected = function () {
    ...
    if (this.credentials.length > 0) {
        this.credentials.forEach(function (credential) {
            ...
            this.socket.write(authRequest.toBuffer());
        }, this);
    }
    ...
    if (!this.watcherManager.isEmpty()) {
        ...
        // Send inner-request without queue
        this.socket.write(setWatchesRequest.toBuffer());
    }
};
...
ConnectionManager.prototype.onSocketTimeout = function () {
    ...
    if (this.socket &&
        ...
        pingRequest = new jute.Request(header, null);
        // Send inner-request without queue
        this.socket.write(pingRequest.toBuffer());
